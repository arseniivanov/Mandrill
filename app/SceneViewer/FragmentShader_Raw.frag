...
layout(set = 2, binding = 6) readonly buffer GridL0G0_Buffer { uint data[]; } neuralRawGridL0G0;
layout(set = 2, binding = 7) readonly buffer GridL0G1_Buffer { uint data[]; } neuralRawGridL0G1;
layout(set = 2, binding = 8) readonly buffer GridL1G0_Buffer { uint data[]; } neuralRawGridL1G0;
layout(set = 2, binding = 9) readonly buffer GridL1G1_Buffer { uint data[]; } neuralRawGridL1G1;
layout(set = 2, binding = 10) readonly buffer GridL2G0_Buffer { uint data[]; } neuralRawGridL2G0;
layout(set = 2, binding = 11) readonly buffer GridL2G1_Buffer { uint data[]; } neuralRawGridL2G1;
layout(set = 2, binding = 12) readonly buffer GridL3G0_Buffer { uint data[]; } neuralRawGridL3G0;
layout(set = 2, binding = 13) readonly buffer GridL3G1_Buffer { uint data[]; } neuralRawGridL3G1;
...
uint unpack_2bit_value(uint packed_dword, uint internal_index) {
    // internal_index is 0-15, which value inside the dword we want.
    uint bit_shift = internal_index * 2;
    return (packed_dword >> bit_shift) & 0x03u;
}

float get_feature_from_packed_grid(uint channel_idx, uvec2 coords, int level_idx, int grid_type) {
    
    // Get the original [C, H, W] of the grid
    uvec3 grid_dims = materialParams.featureGridShapes[level_idx][grid_type].xyz;
    uint C = grid_dims.x;
    uint H = grid_dims.y;
    uint W = grid_dims.z;
    ...
    uint internal_index = linear_2bit_index % 16;
    
    uint packed_dword;
    // Select the correct buffer based on level and grid type
    // This is verbose, but direct.
    if (level_idx == 0 && grid_type == 0) packed_dword = neuralRawGridL0G0.data[dword_index];
    if (level_idx == 0 && grid_type == 1) packed_dword = neuralRawGridL0G1.data[dword_index];
    if (level_idx == 1 && grid_type == 0) packed_dword = neuralRawGridL1G0.data[dword_index];
    if (level_idx == 1 && grid_type == 1) packed_dword = neuralRawGridL1G1.data[dword_index];
    if (level_idx == 2 && grid_type == 0) packed_dword = neuralRawGridL2G0.data[dword_index];
    if (level_idx == 2 && grid_type == 1) packed_dword = neuralRawGridL2G1.data[dword_index];
    if (level_idx == 3 && grid_type == 0) packed_dword = neuralRawGridL3G0.data[dword_index];
    if (level_idx == 3 && grid_type == 1) packed_dword = neuralRawGridL3G1.data[dword_index];

    // 3. Unpack the 2-bit value to get a palette index
    uint palette_index = unpack_2bit_value(packed_dword, internal_index);

    // 4. Look up the final float value in the palette
...
vec4 evaluate_neural_texture(vec2 uv, float lod) {
    ...
    if (num_selections_g0 > 0) {
        ...
        uvec2 p00 = uvec2(mod(p00_int_coord.x, int(grid_dims.x)),         mod(p00_int_coord.y, int(grid_dims.y)));
        uvec2 p10 = uvec2(mod(p00_int_coord.x + 1, int(grid_dims.x)),     mod(p00_int_coord.y, int(grid_dims.y)));
        uvec2 p01 = uvec2(mod(p00_int_coord.x, int(grid_dims.x)),         mod(p00_int_coord.y + 1, int(grid_dims.y)));
        uvec2 p11 = uvec2(mod(p00_int_coord.x + 1, int(grid_dims.x)),     mod(p00_int_coord.y + 1, int(grid_dims.y)));

        for (int i = 0; i < num_selections_g0; ++i) {
            uint channel_to_sample;
            switch(level_idx) {
                case 0: channel_to_sample = uint(grid0l0_channels.indices[i]); break;
                case 1: channel_to_sample = uint(grid0l1_channels.indices[i]); break;
                case 2: channel_to_sample = uint(grid0l2_channels.indices[i]); break;
                case 3: channel_to_sample = uint(grid0l3_channels.indices[i]); break;
            }
            float f00 = get_feature_from_packed_grid(channel_to_sample, p00, level_idx, 0);
            float f10 = get_feature_from_packed_grid(channel_to_sample, p10, level_idx, 0);
            float f01 = get_feature_from_packed_grid(channel_to_sample, p01, level_idx, 0);
            float f11 = get_feature_from_packed_grid(channel_to_sample, p11, level_idx, 0);
    ...
    if (num_selections_g1 > 0) {
        uvec2 grid_dims = materialParams.featureGridShapes[level_idx][1].yz;
        ...
        for (int i = 0; i < num_selections_g1; ++i) {
            ...
            switch(level_idx) {
                case 0: channel_to_sample = uint(grid1l0_channels.indices[i]); break;
                case 1: channel_to_sample = uint(grid1l1_channels.indices[i]); break;
                case 2: channel_to_sample = uint(grid1l2_channels.indices[i]); break;
                case 3: channel_to_sample = uint(grid1l3_channels.indices[i]); break;
            }
            
            float f00 = get_feature_from_packed_grid(channel_to_sample, p00, level_idx, 1);
            float f10 = get_feature_from_packed_grid(channel_to_sample, p10, level_idx, 1);
            float f01 = get_feature_from_packed_grid(channel_to_sample, p01, level_idx, 1);
            float f11 = get_feature_from_packed_grid(channel_to_sample, p11, level_idx, 1);
